...
| 能力 | 实现方式 |
| --- | --- |
| 中文官网白名单 | 每个期刊配置首页、官方域名、栏目和页面选择器；支持静态 HTML 分页与文章详情页。 |
...
本版本的官网采集适用于**可直接取得的静态页面**。需要登录、JavaScript 渲染或特殊接口的期刊，应增加专用适配器；扫描失败会显示为覆盖不完整，不会变成“无新文”。RSS 快照只能代表该订阅源提供的条目，不能证明整个平台已被穷尽。研究设计抽取基于标题和摘要，不能替代全文核读；缺失字段保留为空。
...
在配置的 `chinese_monitor.journals` 中**明确**加入期刊名称、首页、允许的官方域名及每个栏目。CSS 选择器由该官网的真实页面结构确定。对重点期刊设 `priority: critical`，系统会先保存栏目全部新条目，再做相关度分析；低相关新增文仍在周报 B 节巡视。中文新文不会从 CNKI、万方、搜索引擎或 Crossref 发现。
