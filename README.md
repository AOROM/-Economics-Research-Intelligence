...
| 能力 | 实现方式 |
| --- | --- |
| 中文官网白名单 | 预设包含用户指定的 11 本期刊；其中 3 本配置了官方文章页面链接规则，8 本明确标为待接入。支持静态 HTML 分页、官方文章链接模式与详情页。 |
...
本版本的官网采集适用于**可直接取得的静态页面**。需要登录、JavaScript 渲染或特殊接口的期刊，应增加专用适配器；扫描失败或栏目没有匹配到论文链接，会显示为覆盖不完整，不会变成“无新文”。待接入期刊会在周报中逐项列出，但不会声称已扫描。RSS 快照只能代表该订阅源提供的条目，不能证明整个平台已被穷尽。研究设计抽取基于标题和摘要，不能替代全文核读；缺失字段保留为空。
...
当前 11 本中文期刊及接入状态见[中文期刊监控说明](references/chinese-journal-monitoring.md)。其中《世界经济》《金融研究》《财经研究》已有官网栏目和文章链接规则；首次在实际运行环境中扫描前，采集连通性与完整性仍待验证。其它 8 本保留在名单中，标为 `pending`，不会参与扫描。
