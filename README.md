...
面向经济学与金融学研究的英文文献监控项目。当前定时预设只检索《浙江财经大学中外文学术期刊定级管理办法（2020年修订）》中经济与金融领域的 8 本外文 TOP 期刊，按首次观察记录新工作，追踪可核实的版本变化，并生成带来源链接的中文研究雷达。

## 当前可运行能力

| 能力 | 实现方式 |
| --- | --- |
| 英文目标期刊 | 定时预设只保留校级目录中的经济与金融 TOP 8 本。按 ISSN 查询 Crossref 期刊作品元数据，DOI 链接回到出版社页面。 |
...
英文检索依赖出版社向 Crossref 登记的元数据。扫描失败或接口返回异常时，邮件会显示为覆盖不完整，不会写成“无新文”。Crossref 不能替代出版社官网或全文数据库的完整性证明。论文总结明确标注标题、摘要或用户提供正文的材料范围；缺失字段保持待核实。
