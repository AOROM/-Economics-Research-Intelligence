...
6. Write UTF-8 MIME email drafts, HTML previews, plain-text mail, the compatible A–F digest and research map. Interactive draft runs clear pending events after successful export. Delivery runs clear them only after the SMTP server accepts the message. Failed summaries retain a usable extraction and retry state; a failed source never gets a new successful-scan timestamp.
